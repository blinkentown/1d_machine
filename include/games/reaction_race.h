...
class ReactionRaceGame {
 ...
 private:
  ...
  void updateProgressAnimation(uint32_t now);
  uint16_t nextRandom();

  Phase phase_ = Phase::Waiting;
  uint32_t phaseChangedAt_ = 0;
  uint32_t goAt_ = 0;
  uint32_t lastProgressPixelStepAt_ = 0;
  uint16_t randomState_ = 1;
  uint8_t player1Progress_ = 0;
  uint8_t player2Progress_ = 0;
  uint8_t player1RenderProgressPixels_ = 0;
  uint8_t player2RenderProgressPixels_ = 0;
